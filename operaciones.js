export function factorDeParo(factor, xl, xu) {
    ...
    var ea; // error relativo
    var xrNuevo = 0; // para error relativo
    var xrViejo; // para error relativo
    var PosiNega; // Par de números positivo y negativo
    var seraIteracion = /^[0-9]+$/.test(factor); // RegExp busca que en todo el texto solo hayan números, .test(variable) se usa para probar dicho RegExp en la variable regresando un Boolean
    var numeroDeIteraciones = Number(factor); // Convierte el texto en número
    if (seraIteracion) { // En caso de que sí sea por iteracion
        for (var i = 1; i <= numeroDeIteraciones; i++) { // i representa la iteracion
        ...
        var operadorMatch = (_a = factor.match(/^(<=?|>=?|==|!=)/)) === null || _a === void 0 ? void 0 : _a[0];
        var operador = operadorMatch ? operadorMatch.trim() : '';
        ...
        var valorStr = factor.replace(/^(<=?|>=?|==|!=)\s*/, '');
        var valor = parseFloat(valorStr); //Para quitar dicho operador
        var condiciones = {
        ...
        var FDP = (_b = condiciones[operador]) !== null && _b !== void 0 ? _b : condiciones['']; // En base al registro obtiene la operación a usar, en caso de ser indefinido arroja la opción por default, que en este caso es ''
        var iteration = 0; // numero de iteraciones
        do {
...
function obtainPositiveAndNegative(ob) {
    ...
    for (var i = 0; i < ob.length - 1; i++) { // Obtener Xs de los FXs necesarios
        var act = ob[i].fx;
        var sig = ob[i + 1].fx;
        if (act < 0 && sig > 0) {
            res = [ob[i].x, ob[i + 1].x];
